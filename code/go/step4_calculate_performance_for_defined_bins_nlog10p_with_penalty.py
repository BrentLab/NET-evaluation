...
def calculate_performance_for_defined_bins(p_net
    ...
    for nbr_edges_per_reg in l_nbr_edges_per_reg:
        ...
        l_pvalue_nlog10p = -np.log10(l_pvalue)
        l_pvalue_nlog10p_with_penalty = l_pvalue_nlog10p - 2
        if not l_pvalue:
            ...
            l_sum_pvalue.append(sum(l_pvalue_nlog10p_with_penalty))
            l_mean_pvalue.append(mean(l_pvalue_nlog10p_with_penalty))
            l_median_pvalue.append(median(l_pvalue_nlog10p_with_penalty))
            l_nbr_tf.append(len(l_pvalue_nlog10p_with_penalty))
    pool.close()
    pool.join()
